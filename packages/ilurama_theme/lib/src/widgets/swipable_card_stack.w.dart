...
/// A Tinder-Like Swipable Card Widget
class SwipableCard extends StatefulWidget {
  ...
  final CardSwipeCompleteCallback? onDismissed;
  ...
  SwipableCard({
    ...
    final double widthGap = maxWidth - minWidth;
    final double heightGap = maxHeight - minHeight;

    _cardAligns = [];
    _cardSizes = [];

    for (int i = 0; i < _stackNum; i++) {
      _cardSizes.add(Size(minWidth + (widthGap / _stackNum) * i, minHeight + (heightGap / _stackNum) * i));

      switch (orientation) {
        case AmassOrientation.bottom:
          _cardAligns.add(Alignment(0.0, (2.2 / (_stackNum - 1)) * (stackNum - i)));
          break;
...
class _SwipableCardState extends State<SwipableCard> with TickerProviderStateMixin {
  ...
  Widget _buildCard(BuildContext context, int realIndex) {
    ...
    final int index = realIndex - _currentFront;

    if (index == widget._stackNum - 1) {
      return Align(
        alignment: _animationController.status == AnimationStatus.forward
            ? frontCardAlign = CardAnimation.frontCardAlign(_animationController, frontCardAlign,
                    _cardAligns[widget._stackNum - 1], widget._swipeEdge, widget._swipeUp, widget._swipeDown)
                .value
            : frontCardAlign!,
        ...
        onPanUpdate: (DragUpdateDetails details) {
          setState(() {
            if (widget._allowVerticalMovement == true) {
              frontCardAlign = Alignment(frontCardAlign!.x + details.delta.dx * 20 / MediaQuery.of(context).size.width,
                  frontCardAlign!.y + details.delta.dy * 30 / MediaQuery.of(context).size.height);
            } else {
              frontCardAlign =
                  Alignment(frontCardAlign!.x + details.delta.dx * 20 / MediaQuery.of(context).size.width, 0);
  ...
  void _initState() {
    ...
    _animationController = AnimationController(vsync: this, duration: Duration(milliseconds: widget._animDuration));
    _animationController.addListener(() => setState(() {}));
    _animationController.addStatusListener((AnimationStatus status) {
      final int index = widget._totalNum - widget._stackNum - _currentFront;
      if (status == AnimationStatus.completed) {
        ...
        if (orientation != CardSwipeOrientation.recover) {
          if (widget.onDismissed != null) {
            widget.onDismissed!(orientation, index);
          }
          changeCardOrder();
        }
      }
...
class CardAnimation {
  static Animation<Alignment> frontCardAlign(
    ...
    if (_SwipableCardState._trigger == TriggerDirection.none) {
      endX = beginAlign!.x > 0
    ...
    } else if (_SwipableCardState._trigger == TriggerDirection.left) {
      endX = beginAlign!.x - swipeEdge;
      endY = beginAlign.y + 0.5;
    }
    /* Trigger Swipe Up or Down */
    else if (_SwipableCardState._trigger == TriggerDirection.up ||
        _SwipableCardState._trigger == TriggerDirection.down) {
      final beginY = _SwipableCardState._trigger == TriggerDirection.up ? -10 : 10;
  ...
  static Animation<double> frontCardRota(AnimationController controller, double beginRot) {
    return Tween(begin: beginRot, end: 0.0).animate(CurvedAnimation(parent: controller, curve: Curves.easeOut));
  }
